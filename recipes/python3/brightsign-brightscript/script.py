...
MAX_STATUS_FIELD_LENGTH = 256

def boundedStatusField(value):
  text = str(value)
  if len(text) <= MAX_STATUS_FIELD_LENGTH:
    return text
  return '%s... [%s characters truncated]' % (
      text[:MAX_STATUS_FIELD_LENGTH],
      len(text) - MAX_STATUS_FIELD_LENGTH)
...
def sendGet(value, refresh=True):
  global fullAddress, _lastReceive
  if is_blank(fullAddress):
    console.warn("Brightsign address is not configured")
    return
  try:
  ...
  if refresh:
    call(playerStatusGet, delay=0.25)
...
def send_udp_string(msg):
  ...
  try:
    ...
    sock.sendto(payload, (ipAddress, udpPort))
  except socket.error as error:
    console.error("UDP send failed: %s" % error)
  finally:
    if sock:
      sock.close()
...
def playerStatusGet():
  ...
  try:
    ...
    console.warn("Status request failed: %s" % error)
    return
  ...
  try:
    status_decode = json.loads(resp.content)
    model = boundedStatusField(status_decode['model'])
    volume = boundedStatusField(status_decode['volume'])
    serial = boundedStatusField(status_decode['serialNumber'])
    video_mode = boundedStatusField(status_decode['videomode'])
    sleep = str(status_decode['sleep'])
    playing = str(status_decode['playing'])
    muted = str(status_decode['muted'])
  except Exception as error:
    console.warn("Invalid Brightsign status response: %s" % error)
    return

  _lastReceive = system_clock()
  lookup_local_event('Model').emit(model)
  lookup_local_event('Volume').emit(volume)
  lookup_local_event('Serial').emit(serial)
  lookup_local_event('VideoMode').emit(video_mode)
  lookup_local_event('volume').emit(volume)

  if sleep == "true":
    lookup_local_event('Power').emit('Off')
    if lookup_local_event('DesiredPower').getArg() == "On":
      sendGet("/playback?sleep=false", refresh=False)
  elif sleep == "false":
    lookup_local_event('Power').emit('On')
    if lookup_local_event('DesiredPower').getArg() == "Off":
      sendGet("/playback?sleep=true", refresh=False)

  if playing == "true":
    lookup_local_event('Playback').emit('Playing')
    if lookup_local_event('DesiredPlayback').getArg() == "Paused":
      sendGet("/playback?playback=pause", refresh=False)
  elif playing == "false":
    lookup_local_event('Playback').emit('Paused')
    if lookup_local_event('DesiredPlayback').getArg() == "Playing":
      sendGet("/playback?playback=play", refresh=False)

  if muted == "true":
    lookup_local_event('Mute').emit('On')
    if lookup_local_event('DesiredMute').getArg() == "Off":
      sendGet("/mute?unmute", refresh=False)
  elif muted == "false":
    lookup_local_event('Mute').emit('Off')
    if lookup_local_event('DesiredMute').getArg() == "On":
      sendGet("/mute?mute", refresh=False)
...
def main(arg = None):
  ...
  playerStatus_timer.start()
  nodeStatus_timer.start()
...
playerStatus_timer = Timer(playerStatusGet, status_check_interval, stopped=True)
nodeStatus_timer = Timer(nodeStatusCheck, status_check_interval, stopped=True)
