'''Synthetic Nodel v2 fixture for comparison with the nano runtime profile.'''
...
def parse_echo(data, transport):
    ...
    if not value.startswith('SEQ '):
        console.warn('Ignoring malformed %s echo: %r' % (transport, value))
        return None
    try:
        seq = int(value[4:])
    except ValueError:
        console.warn('Ignoring malformed %s sequence: %r' % (transport, value))
        return None
    if seq < 1:
        console.warn('Ignoring non-positive %s sequence: %s' % (transport, seq))
        return None
    return seq
